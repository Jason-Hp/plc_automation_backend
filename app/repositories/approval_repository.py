...
class ApprovalRepository:
    ...
    def get_all_approvals(self) -> List[Approval]:
        ...
        resp = self._client.table("approvals").select("*").execute()
        return [Approval.model_validate(r) for r in (resp.data or [])]

    def get_approvals_by_type(self, approval_type: str) -> List[Approval]:
        """Retrieve approvals filtered by type."""
        resp = self._client.table("approvals").select("*").eq("type", approval_type).execute()
        return [Approval.model_validate(r) for r in (resp.data or [])]

    def get_approvals_by_status(self, is_approved: bool) -> List[Approval]:
        """Retrieve approvals filtered by approval status."""
        resp = self._client.table("approvals").select("*").eq("is_approved", is_approved).execute()
        return [Approval.model_validate(r) for r in (resp.data or [])]
    ...
    def get_approvals_by_requester(self, requester: str) -> List[Approval]:
        ...
        resp = self._client.table("approvals").select("*").eq("requester", requester).execute()
        return [Approval.model_validate(r) for r in (resp.data or [])]
